...
/*
 * Keep this type local to the summary component.
 *
 * paid_by is returned by /api/expenses and is required by
 * the Payment Activity section below.
 */
type SummaryExpense = {
  id: string;
  trip_id: string;
...
function tripDates(trip: Trip) {
  ...
  if (trip.start_date && trip.end_date) {
    return `${formatDate(trip.start_date)} – ${formatDate(
      trip.end_date
    )}`;
  }
...
export default function SpendingSummary({
  ...
  const [expenses, setExpenses] = useState<SummaryExpense[]>([]);
  ...
  async function loadSummary(
    ...
    try {
      ...
      /*
       * Explicitly normalize the API response here.
       *
       * This guarantees paid_by exists on every SummaryExpense
       * object even if an unexpected/null value comes back.
       */
      const normalizedExpenses: SummaryExpense[] = (
        data.expenses || []
      ).map((expense) => ({
        id: expense.id,
  ...
  const payerData = useMemo(() => {
    ...
    expenses.forEach((expense) => {
      /*
       * paid_by is now explicitly part of SummaryExpense.
       */
      const member = members.find(
        (item) => item.id === expense.paid_by
      );
        ...
        <div className="mt-5 flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-[15px] leading-7 text-[#68655d]">
              See where this trip&apos;s recorded
              expenses are going, which categories
              drive the total, and how every number
              is calculated.
            </p>
          </div>
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-[#817d74]">
                  Switch between summaries without
                  leaving the ledger.
                </p>
              </div>
                  ...
                  <section className="mt-6 grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
                    <div className="rounded-[30px] border border-[#292a25]/10 bg-[#f8f5ec] p-6 shadow-[0_15px_50px_rgba(40,40,30,.04)]">
                      ...
                      <p className="mt-4 text-xs leading-5 text-[#8b877d]">
                        Category percentage =
                        category amount ÷ total spend ×
                        100. The pie chart uses the same
                        calculation, so the slices always
                        add up to 100%.
                      </p>
                    </div>
                  ...
                  {payerData.length > 0 && (
                    <section className="mt-6 rounded-[30px] border border-[#292a25]/10 bg-[#f8f5ec] p-6 shadow-[0_15px_50px_rgba(40,40,30,.04)]">
                      ...
                      <p className="mt-2 max-w-2xl text-sm leading-6 text-[#817d74]">
                        This is the amount recorded as
                        paid by each person. It is different
                        from their final settlement balance.
                      </p>
...
function MetricCard({
  ...
  return (
    <div className="rounded-[24px] border border-[#292a25]/10 bg-[#f8f5ec] p-5 shadow-[0_15px_50px_rgba(40,40,30,.04)]">
      ...
      <p className="mt-1 text-xs leading-5 text-[#8b877d]">
        {detail}
      </p>
    </div>
  );
  ...
  return (
    <div className="rounded-[30px] border border-dashed border-[#292a25]/15 bg-[#f8f5ec] px-6 py-16 text-center shadow-[0_15px_50px_rgba(40,40,30,.04)]">
      ...
      <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-[#817d74]">
        Create a trip and add expenses to start
        building its spending summary.
      </p>
